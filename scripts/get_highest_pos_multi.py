#!/usr/bin/env python

import numpy as np

import rospy
import sensor_msgs.point_cloud2 as pc2
import tf.transformations

from geometry_msgs.msg import Pose, PoseArray
from jsk_topic_tools import ConnectionBasedTransport
from sensor_msgs.msg import PointCloud2
from sklearn.decomposition import PCA

class DetectGraspablePosesPcabase(ConnectionBasedTransport):

    def __init__(self):

        rospy.logwarn("This node is experiential one.")

        super(DetectGraspablePosesPcabase, self).__init__()

        self.pub_target_poses = self.advertise("~output", PoseArray, queue_size=1)

    def subscribe(self):
        rospy.Subscriber('~input', PointCloud2, self.callback)

    def unsubscribe(self):
        self.sub.unregister()

    def callback(self, point_cloud):

        print("callback")

        points = np.array(list(pc2.read_points(point_cloud, skip_nans=True)))
        if points.size == 0:
          pub_msg = PoseArray()
          pub_msg.header = point_cloud.header
          self.pub_target_poses.publish(pub_msg)
          return

        points_sorted = points[points[:,3].argsort(), :]

        points_list = [] 

        ps = []
        now_index = 0
        for i in range(len(points_sorted)):
          index = points_sorted[i][3]

          if now_index == index:
            ps.append(points_sorted[i])

          else:
            points_list.append(ps)
            ps = []
            now_index = index

        points_list.append(ps)

        
        pub_msg = PoseArray()
        pub_msg.header = point_cloud.header

        for i in range(len(points_list)):

          target_points = np.array(points_list[i])
          target_points_sorted =  target_points[target_points[:,2].argsort(), :]
          pose = Pose()

          pose.position.x = target_points_sorted[-1][0]
          pose.position.y = target_points_sorted[-1][1]
          pose.position.z = target_points_sorted[-1][2]

          pose.orientation.x = 0
          pose.orientation.y = 0
          pose.orientation.z = 0
          pose.orientation.w = 1

          pub_msg.poses.append(pose)

        self.pub_target_poses.publish(pub_msg)

if __name__ == '__main__':
    print("test")
    rospy.init_node('get_highest_pos')
    DetectGraspablePosesPcabase()
    rospy.spin()
